...
export default function ExpenseForm() {
  ...
  // Load trip settings to get custom categories
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await postExpense({ tripId: activeTripId, date, description: description.trim(), whoPaid: whoPaid.trim(), category, cost: costNumber });
      ...
      <Card>
        <CardContent>
          <Stack spacing={2}>
            ...
            <TextField label="Who paid?" value={whoPaid}
            ...
            <TextField label="Cost" type="number" value={cost}
              onChange={(e) => setCost(e.target.value)} inputProps={{ min: 0, step: "0.01" }} fullWidth />
